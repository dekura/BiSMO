###
 ...
 # @LastEditTime: 2023-11-10 18:05:58
...
for i in $unroll_steps; do
    echo "Starting bismo for: problems ["$problems_type"] mask1 unroll_steps="$i
    $python src/bilevel.py module.device_id=$device_id problems.unroll_steps=$i
done
