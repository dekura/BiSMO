...
class MOLitModule(LightningModule):
    ...
    def model_step_old(self):
        ...
        pvb = (
            (self.criterion(RIlist[0], RIlist[1]) + self.criterion(RIlist[2], RIlist[1]))
            * self.mask.target_data.shape[0]
            * self.mask.target_data.shape[1]
        )
    ...
    def model_step(
        ...
        pvb = (
            (self.criterion(RI_norm, RI_min) + self.criterion(RI_norm, RI_max))
            * self.mask.target_data.shape[0]
            * self.mask.target_data.shape[1]
        )
        loss = l2 * self.hparams.weight_l2 + pvb * self.hparams.weight_pvb
    ...
    def test_step(self, batch: Any, batch_idx: int) -> None:
        ...
        AI_folder = save_img_folder / "moed_AI"
        AI_folder.mkdir(parents=True, exist_ok=True)

        RI_folder = save_img_folder / "moed_RI"
        RI_folder.mkdir(parents=True, exist_ok=True)

        mask_folder = save_img_folder / "moed_mask"
