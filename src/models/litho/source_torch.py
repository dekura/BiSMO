...
if __name__ == "__main__":
    ...
    from source import Source as NP_Source
    np_s = NP_Source()
    np_s.type = "annular"
    np_s.sigma_in = 0.6
    np_s.sigma_out = 0.8
    ...
    # torch_arr_bound(s.data, "s.data")
    # arr_bound(np_s.data, "numpy s.data")

    # torch_arr_bound(s.mdata, "s.mdata")
    # arr_bound(np_s.mdata, "numpy s.mdata")

    torch_arr_bound(s.spatMutualData, "s.spatMutualData")
    arr_bound(np_s.spatMutualData, "numpy s.spatMutualData")


    torch_arr_bound(torch.fft.ifftshift(s.mdata), "torch.fft.ifftshift(s.mdata)")
    arr_bound(np.fft.ifftshift(np_s.mdata), "np.fft.ifftshift(np_s.mdata)")
    # delta_np_torch(np.fft.ifftshift(np_s.mdata), torch.fft.ifftshift(s.mdata))

    # delta_np_torch(np_s.data, s.data)
