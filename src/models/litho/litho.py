...
class LithoSim(nn.Module): # Mask -> Aerial -> Printed
    def __init__(
            self, 
            KernelDir: str = './kernel', 
            KernelNum: int = 24, 
            TargetDensity: float = 0.225, 
            PrintThresh: float = 0.5,
            PrintSteepness: float = 50.0, 
            DoseMax: float = 1.02, 
            DoseMin: float = 0.98, 
            DoseNom: float = 1.00, 
            ): 
        super(LithoSim, self).__init__()
        self._KernelDir = KernelDir
        self._KernelNum = KernelNum
        self._TargetDensity = TargetDensity
        self._PrintThresh = PrintThresh
        self._PrintSteepness = PrintSteepness
        self._DoseMax = DoseMax
        self._DoseMin = DoseMin
        self._DoseNom = DoseNom
        ...
        # Read the kernels
        self._kernels = {"focus": Kernel(self._KernelDir), 
                         "defocus": Kernel(self._KernelDir, defocus=True),
                         "CT focus": Kernel(self._KernelDir, conjuncture=True),
                         "CT defocus": Kernel(self._KernelDir, defocus=True, conjuncture=True),
                         "combo focus": Kernel(self._KernelDir, combo=True),
                         "combo defocus": Kernel(self._KernelDir, defocus=True, combo=True),
                         "combo CT focus": Kernel(self._KernelDir, conjuncture=True, combo=True),
                         "combo CT defocus": Kernel(self._KernelDir, defocus=True, conjuncture=True, combo=True)}


    def forward(self, mask): 
        aerialNom = _LithoSim.apply(mask, self._DoseNom, 
                                    self._kernels["focus"].kernels, self._kernels["focus"].scales, self._KernelNum, 
        ...
        aerialMax = _LithoSim.apply(mask, self._DoseMax, 
                                    self._kernels["focus"].kernels, self._kernels["focus"].scales, self._KernelNum, 
                                    self._kernels["combo CT focus"].kernels, self._kernels["combo CT focus"].scales, 1, 
                                    self._kernels["combo focus"].kernels, self._kernels["combo focus"].scales, 1)
        aerialMin = _LithoSim.apply(mask, self._DoseMin, 
                                    self._kernels["defocus"].kernels, self._kernels["defocus"].scales, self._KernelNum, 
        ...
        printedNom = torch.sigmoid(self._PrintSteepness * (aerialNom - self._TargetDensity))
        printedMax = torch.sigmoid(self._PrintSteepness * (aerialMax - self._TargetDensity))
        printedMin = torch.sigmoid(self._PrintSteepness * (aerialMin - self._TargetDensity))
...
if __name__ == '__main__':
    ...
    m.poly2tensor()
    image = m.image()
    image = torch.tensor(image > 0.0, dtype=torch.float32, device=torch.device("cuda"))
    # k  = Kernel()
    litho = LithoSim()
