...
# import numpy as np
import torch
# from src.models.litho.zernike_torch import zerniken
...
if __name__ == "__main__":
    ...
    from lens import LensList as NP_LensList
    from lens import Lens as NP_Lens

    np_lens_list = NP_LensList()
    np_lens_list.na = 0.85
    np_lens_list.focusList = [-50, 0, 50]
    np_lens_list.focusCoef = [0.5, 1, 0.5]
    np_lens_list.calculate()
    ...
    for i in range(len(lens_list.fDataList)):
        torch_arr_bound(lens_list.fDataList[i], f"lenslist.fDataList[{i}]")
        arr_bound(np_lens_list.fDataList[i], f"np_lenslist.fDataList[{i}]")