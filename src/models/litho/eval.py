...
# from src.utils.glp import Design
...
def epecheck(mask, 
             ...
             vposes, 
             hposes, 
             MIN_EPE_CHECK_LENGTH: int = 80, 
             EPE_CHECK_START_INTERVEL: int = 40, 
             EPE_CHECK_INTERVEL: int = 40, 
             ):
    '''
...
class Evaluate:
    def __init__(self, 
        ...
        self.test = Basic(thresh)
        self.epeCheck = EPEChecker(thresh)
        self.shotCount = ShotCounter(thresh)
    ...
    def eval(self, 
        ...
        l2, pvb = self.test.run(mask, target, scale=scale)
        epeIn, epeOut = self.epeCheck.run(mask, target, scale=scale)
        epe = epeIn + epeOut
        nshot = self.shotCount.run(mask, shape=(512, 512)) if shots else -1

        return l2, pvb, epe, nshot



# def evaluate(mask, target, scale=1, shots=False, verbose=False): 
#     test = Basic(0.5)
#     epeCheck = EPEChecker(0.5)
#     shotCount = ShotCounter(0.5)
...
#     else: 
#         # ref = Design(targetfile, down=1)
#         ref = Mask(targetfile, down = 1)
#         # ref.poly2tensor(2048, 2048, 0, 0)
#         ref.openGLP()
