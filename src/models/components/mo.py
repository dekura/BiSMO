...
class MO_Module(nn.Module):
    ...
    def init_freq_domain_on_device(self) -> None:
        ...
        self.source_fx1d = torch.reshape(self.s_fx, (-1, 1)).to(device)
        self.source_fy1d = torch.reshape(self.s_fy, (-1, 1)).to(device)
    ...
    def update_mask_value(self) -> None:
        ...
        # self.mask.maskfft()
        self.mask_value.to(self.device)
        ...
        print(f"maskfvalue device: {self.mask_fvalue_min.device}")
    ...
    def forward(self,
                source_value: torch.Tensor) -> tuple[list, list]:
