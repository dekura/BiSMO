...
class MO(ImplicitProblem):
    def __init__(self, 
                 ...
                 mask: Mask, 
                 name: str = 'outer', 
                 config: Config = Config(type = 'darts', log_step=1, first_order=True, retain_graph=True), 
                 module = None, 
                 optimizer = None, 
                 scheduler = None, 
    ...
    def init_freq_domain_on_device(self) -> None:
        ...
        self.s.data = self.source.data.to(device)
    ...
    def init_mask_params(self) -> None:
        # learnable, [-1, 1]
        self.mask_params = torch.nn.Parameter(self.mask.data.float())
    ...
    def forward(self) -> tuple[list, list]:
        ...
        # RI_min = torch.where(self.RI_list[0] > 0.5, 1.0, 0.0).float()
        # RI_norm = torch.where(self.RI_list[1] > 0.5, 1.0, 0.0).float()
        # RI_max = torch.where(self.RI_list[2] > 0.5, 1.0, 0.0).float()

        # RI_pvb = torch.where(RI_min != RI_max, 1.0, 0.0).float()

        # l2 = self.criterion(self.RI_list[1], self.mask.target_data.float())
        # pvb = self.criterion(self.RI_list[1], self.RI_list[0]) + self.criterion(self.RI_list[1], self.RI_list[2])
...
class SO(ImplicitProblem):
    def __init__(self, 
                 ...
                 mask: Mask, 
                 name: str = 'inner', 
                 config: Config = Config(type="cg", cg_iterations=3, cg_alpha=1, unroll_steps=100), 
                 module = None, 
                 optimizer = None, 
                 scheduler = None, 
                 train_data_loader = None, 
