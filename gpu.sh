...
###
 ...
 # @LastEditTime: 2023-10-06 15:59:01
...
for type in $source_list; do
    for i in $(seq 1 1); do
        ...
        $python src/smo.py experiment=smallsource task_name="smo_"$type mask.layout_path=$layout_path mask.target_path=$layout_path source.source_type=$type
    done
done
